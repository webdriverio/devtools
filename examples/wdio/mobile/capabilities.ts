...
const CLOCK_APP = {
  ...
  ios: { 'appium:bundleId': 'com.apple.mobiletimer' }
}
...
export function mobileCapabilities(): Record<string, unknown> {
  ...
  return {
    ...
    // iOS needs the simulator named explicitly and it is per-machine, so it
    // comes from the environment: `xcrun simctl list devices` shows yours.
    ...(IOS
      ? {

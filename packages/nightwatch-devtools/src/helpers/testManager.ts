...
export class TestManager {
  ...
  detectTestBoundary(currentNightwatchTest: any): string {
    ...
    if (
      this.lastKnownTestName &&
      currentTestName !== this.lastKnownTestName &&
      currentTestName !== DEFAULTS.TEST_NAME
    ) {
      const testFile =
        currentNightwatchTest.module?.split('/').pop() || DEFAULTS.FILE_NAME
      const currentSuite = this.testReporter.getCurrentSuite()

      if (currentSuite) {
        const prevTest = this.findTestInSuite(
          currentSuite,
          this.lastKnownTestName
        )
        if (prevTest && prevTest.state === TEST_STATE.RUNNING) {
          const prevTestCase =
            currentNightwatchTest.results?.testcases?.[this.lastKnownTestName]
          ...
          if (prevTestCase) {
            ...
            this.updateTestState(
              ...
              testState,
              new Date(),
              parseFloat(prevTestCase.time || '0') * 1000
            )
    ...
    suite.tests.forEach((test: any) => {
      if (test.state === TEST_STATE.RUNNING && test.start) {
        ...
        if (testcase) {
          ...
          this.updateTestState(
            test,
            TEST_STATE.SKIPPED as TestStats['state'],
            new Date(),
            0
          )
        }

...
- **Headless Chrome stops activating clicks after the first test in a session — NOT capture-related, and NOT fixed in 151.** A click dispatches but never navigates, so a live-mode example fails on its *second* test with a missing-element error that looks like a devtools bug. `browser.reloadSession()` between tests works around it; a fresh session restores input. Measured 2026-08-07 on **Chrome 151.0.7922.109** with bare `--headless` (i.e. new headless), on the WDIO mocha example: the invalid-username test fails **3 of 3** as the second test in a session, **passes when run alone** (`--mochaOpts.grep`), and still fails with the **devtools service removed entirely** — after the submit click the browser is still on `/login` with no `#flash` anywhere in the page source, i.e. the form was never submitted. `browser.deleteCookies()` before the second test does **not** help, so it is not leftover auth state either. Earlier revisions of this entry claimed a Chrome 150 regression "fixed in 151" triggered by capture traffic, and advised pinning `browserVersion` to 149 — all three claims are unsupported: the symptom reproduces on 149, 150 and 151, with and without the plugin, across all three adapters. Root cause still unidentified; do not attribute the next sighting to capture traffic or to a browser version without re-measuring.
  - **The Nightwatch example's intermittent "logout click does nothing" was a SEPARATE bug, and screenshot polling was its DOMINANT cause — but not its only one.** Do not attribute it to the Chrome 150 regression: it reproduced on Chrome **149 and 150 alike**. Equally, do not treat the poll gate as a full fix: a residual ~25% failure rate survives with polling disabled outright, so the earlier "0 of 4 runs fail with the recorder off" reading was small-sample luck and something else is still unidentified. `trace.network` carries **no request for the destination** and the DOM anchor count equals the documents actually visited, i.e. the trace *correctly* reports that the page never navigated — so it was never an anchor-attribution bug. A `GET /screenshot` reaching chromedriver inside a click, between its coordinate computation and its dispatch, makes the click report success while activating nothing. `core/input-dispatch.ts` `InputDispatchGate` now suppresses a poll tick while an input-dispatching command is in flight (`beginInputDispatch(command)` at the adapter's command hook, `isInputDispatchInFlight()` in `ScreencastRecorderBase`'s tick), which classifies commands via shared `isInputDispatchingCommand`. That is derived from `ACTION_MAP`/`POINTABLE_METHODS`/`KEYBOARD_COMMANDS`, **plus** `UNMAPPED_POINTER_COMMANDS` — which is an explicit list of Nightwatch's low-level mouse API (`moveToElement`, `mouseButton*`, `rightClick`, `submitForm`), i.e. genuinely per-adapter, kept out of `ACTION_MAP` so it doesn't add timeline noise. Adding those to `ACTION_MAP` would make the derivation total; until then the list is the exception, not a claim of derivation.
    - Measured on the BDD example (interleaved arms, one build, `filmstrip` on so the recorder polls at 200 ms): **polling with no gate 5/8 runs failed, polling with the gate 3/28 across four blocks (1/8, 2/8, 0/6, 0/6), polling disabled entirely 2/8.** So the gate brings polling down to the no-recorder floor; the ~25% residual is present with *zero* screenshot traffic and is therefore not the recorder.
    - **The residual is NOT the Chrome 150 regression either, and Selenium is NOT immune to it.** Measured 2026-08-06 on **Chrome 151** (i.e. past the version that entry says fixes it), driving the Selenium cucumber example with a logout step: one run navigated normally, the next left the URL unchanged until a 10 s `until.urlContains('/login')` wait expired — and the destination DOM anchor is stamped at **19:03:43.979 against a wait that timed out at ~19:03:43.9**, so the navigation was *delayed by the length of the wait*, not dropped. Selenium on Chrome uses push-based CDP screencast and never polls, so no screenshot traffic is involved. Whatever this is, it is cross-adapter, survives the poll gate, and is not explained; do not attribute the next dropped-click sighting to either the poller or Chrome 150 without re-measuring. The trace confirms the mechanism directly: a fixed run carries `/logout` in `trace.network`, 5 DOM anchors and 24 action rows, against no `/logout`, 4 anchors and 23 rows pre-fix.
    - Bounded on purpose (`INPUT_DISPATCH_MAX_HOLD_MS`, 3 s): a failing Nightwatch test discards the rest of its command queue, so the capture callback that closes a window never fires and an unbounded gate would stall the recorder for the rest of the run. Cost of the gate is dropped frames — ~25-35% fewer in this suite (82-134 → 65-93).
    - **Gaps.** (a) Nightwatch opens the window at *enqueue*, the only hook the adapter has; an awaited call has enqueue ≈ execution, but a command queued behind a long wait can see its window expire before it runs. (b) `browser.element(...).click()` chain style isn't intercepted by `BrowserProxy` at all, so it can't be gated. (c) All three adapters now open the window — Selenium in `driverPatcher.ts` `makeWrappedMethod` (closed at the top of `settle`, which runs on resolve, reject and sync throw alike), the service across `beforeCommand`/`afterCommand` plus `resetStack` so a window stranded by an unpaired command can't bleed into the next test. **Only Nightwatch's wiring is verified against a real browser**; Selenium's and the service's matter only on their non-Chrome polling paths (Chrome uses push-based CDP screencast and never polls), and neither was exercised with a non-Chrome driver — they are unit-tested wiring, not measured fixes. The service holds its closer in a single slot because WDIO serialises commands per session; a concurrent pair (multiremote) closes the earlier window early, degrading to unguarded polling rather than stalling the recorder.
...
- **A Selenium assertion's target comes from VALUE PROVENANCE, not from the assert call.** node:assert takes values, so `assert.equal(await el.getText(), …)` names no element — `selenium-devtools/src/helpers/element-locators.ts` keeps a value→locator registry beside the handle WeakMap (most-recent-producer-wins, 200 keys, ≤256 chars, primitives only), fed from `driverPatcher`'s settle path, and `assertPatcher.resolveAssertTarget` walks the **raw** args (sanitizing destroys handle identity) through core's optional `patchNodeAssert(…, resolveAssertTarget)` hook onto the shared `CommandLog.selector`. A **driver-level** read records its value as belonging to no element (a `null` sentinel) rather than leaving an older element's claim standing — measured on the mocha example, where the `h1` text and the document title are both "Example Domain", so without it a page-title assertion inherited the `h1` box. Residual: boolean reads are weakly discriminating, so a value collision with no intervening producer can mis-attribute. **`after.point` is deliberately not extended to assert rows** — `resolveActionPoint` gates on `class:'Element'` + `POINTABLE_METHODS`, so WDIO's folded assertion rows get none either; a point draws a click marker an assertion never earned.
- **Two mechanisms resolve a command's target selector, and the WDIO one is wrong for interleaved handles.** Selenium keys a `WeakMap` on handle identity (`WebElement.id_` is a promise, so no id is readable when a command is invoked); the service uses a mutable last-selector in `service/src/command-selectors.ts`, which stamps the wrong locator for `const a = await $('#a'); const b = await $('#b'); await a.click()` (the Selenium side is covered in `selenium-devtools/tests/element-locators.test.ts`; the WDIO failure itself has **no** test — it is an unverified reading of `command-selectors.ts`). They cannot share a registry — WDIO's hook sees a *serialized* handle carrying an id string and never the live object, Selenium has the object and no readable id. Unifying would need the policy parameterized rather than the storage shared; until then the WDIO path is knowingly wrong in that case.

...
- **Nightwatch's page-side drain must not go through `browser.execute`.** `browser.*` are QUEUED commands, so the drain used to be deferred 200 ms via `setTimeout` just to stay off Nightwatch's callback stack. That delay lost the outgoing page's field edits whenever the next command navigated inside the window — the `#password` fill's `value` mutations never reached the trace at all. The drain, the collector-ready probe and the injection now go over `helpers/webdriverHttp.ts`, so they touch no queue and fire synchronously from the command callback; because a driver serialises requests per session, a drain issued there is served *before* the next queued command.

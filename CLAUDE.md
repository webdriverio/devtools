...
- **Chrome 150 headless drops trusted input once capture traffic is running** — a click dispatches but never navigates, so a live-mode example fails on its second test with a missing-element error that looks like a devtools bug. It is a browser regression (fixed in 151), not fixable client-side; `browser.reloadSession()` between tests works around it. Pin the example's `browserVersion` to 149 when reproducing live-mode behaviour on a 150 machine.
  - **The Nightwatch example's intermittent "logout click does nothing" is a SEPARATE, unresolved issue — do not attribute it to this.** Measured: it reproduces on Chrome **149 and 150 alike** (so it is not the 150 regression and pinning does not fix it), it **disappears when the plugin is removed** (same suite, same capabilities, no capture: 0 timeouts and the logout navigates), and with capture enabled the same code both passes and fails across runs. A `browser.refresh()` before the click, `--headless=new`, and `browser.end()` between tests were each tried and none fixed it. So capture traffic intermittently interferes with input dispatch; the mechanism is not yet identified and the earlier "trusted input" explanation was wrong.
    - Diagnosing it: `trace.network` carries **no request for the destination** and the DOM anchor count equals the documents actually visited — i.e. the trace is *correctly* reporting that the page never navigated, so this is not an anchor-attribution bug and chasing it as one wastes time. A synthetic in-page click (`execute('…click()')`) does navigate, which narrows it to the WebDriver click path but not further. Next step would be bisecting the capture features (per-action snapshot probes, the post-command drains, the screencast polling) across repeated runs, since a single run per configuration cannot distinguish flaky from fixed.
